...
const BUTTON_STYLE_SYSYEM = {
  ...
  createFontStyle: (size: SizeKey, usage: Usage) => {
    const fontStyles: Record<Usage, ReturnType<typeof css>> = {
      text: css`
        font-size: ${BUTTON_FONTS.text[size as TextSize].fontSize};
        font-weight: ${BUTTON_FONTS.text[size as TextSize].fontWeight};
        line-height: ${BUTTON_FONTS.text[size as TextSize].lineHeight};
      `,
  ...
  createAdditionalStyle: (usage: Usage, size: SizeKey) => {
